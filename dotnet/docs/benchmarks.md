...
- `Apex.DriverBenchmarks` uses BenchmarkDotNet for .NET codec allocations and Apex/Npgsql query, prepared-query, and streaming workloads, plus Apex/MySqlConnector query, prepared-query, streaming, borrowed-reader, pipelining, and repeated-string workloads (`MySqlBenchmarks`, `MySqlPipeliningBenchmarks`).
- `dotnet/benchmarks/java` uses JMH for equivalent Vert.x PostgreSQL query workloads, plus equivalent Vert.x MySQL query, prepared-query, streaming, collector, pipelining, and repeated-string workloads (`MySqlBenchmarks`, `MySqlPipeliningBenchmarks`).
...
| `APEX_BENCH_DATABASE` | Selects `postgres` or `mysql` | `postgres` |
...
| `APEX_BENCH_WORKLOAD` | `query`, `stream100`, `borrowed100`, `pipeline`, or `string100` | `query` |

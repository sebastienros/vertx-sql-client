...
public class MsSqlBenchmarks
{
  ...
  public async Task<int> MicrosoftSimpleQueryAsync() =>
    Convert.ToInt32(await _microsoftSimple.ExecuteScalarAsync());
  ...
  public async Task<int> MicrosoftPreparedQueryAsync() =>
    Convert.ToInt32(await _microsoftPrepared.ExecuteScalarAsync());
  ...
  {
    int sum = 0;
    await foreach (SqlRow row in _apex.StreamAsync(RowsSql, fetchSize: 16))
    {
      sum += row.Get<int>(0);
    }
  ...
  {
    int sum = 0;
    await using ISqlRowReader reader = await _apex.ExecuteReaderAsync(RowsSql);
  ...
  {
    await using SqlDataReader reader = await _microsoftStrings.ExecuteReaderAsync();
    int count = 0;
    while (await reader.ReadAsync())
    {
  ...
  {
    int count = 0;
    await foreach (SqlRow row in _apex.StreamAsync(StringsSql, fetchSize: 16))
    {

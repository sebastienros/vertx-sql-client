...
public sealed class MySqlPacketReaderTests
{
  ...
  public async Task ReadsSinglePacketSmallerThanFrameLimit()
  {
    ...
    byte[] frame = CreateFrame(sequence: 3, [1, 2, 3, 4]);

    await pipe.Writer.WriteAsync(frame);

    using MySqlPacket packet = await reader.ReadAsync(CancellationToken.None);
  ...
  public async Task ReadsPacketSplitAcrossWrites()
  {
    Pipe pipe = new();
    MySqlPacketReader reader = new(pipe.Reader);
    byte[] frame = CreateFrame(sequence: 0, [10, 20, 30, 40, 50]);

    Task<MySqlPacket> pending = reader.ReadAsync(CancellationToken.None).AsTask();
    await pipe.Writer.WriteAsync(frame.AsMemory(0, 3));
    Assert.IsFalse(pending.IsCompleted);
    await pipe.Writer.WriteAsync(frame.AsMemory(3));

    using MySqlPacket packet = await pending;
  ...
  public async Task ReassemblesExactMaximumFramePayloadFollowedByZeroLengthFrame()
  {
    ...
    Pipe pipe = new();
    MySqlPacketReader reader = new(pipe.Reader);
    byte[] first = CreateFrame(sequence: 0, new byte[MySqlProtocol.MaximumFramePayloadLength]);
    byte[] second = CreateFrame(sequence: 1, []);
  ...
  public async Task ReassemblesPayloadSplitAcrossTwoMaximumFrames()
  {
    Pipe pipe = new();
    MySqlPacketReader reader = new(pipe.Reader);
    byte[] firstPayload = CreatePattern(MySqlProtocol.MaximumFramePayloadLength, seed: 1);
    byte[] secondPayload = CreatePattern(128, seed: 2);
    byte[] first = CreateFrame(sequence: 0, firstPayload);
    byte[] second = CreateFrame(sequence: 1, secondPayload);
  ...
  {
    (Stream client, Stream server) = CreateConnectedStreams();
    await using (client)
    await using (server)
    {
      byte[] header = { 0, 0, 0, 9 };
  ...
  public async Task ReadsPacketFromStreamDirectly()
  {
    ...
    await using (server)
    {
      byte[] frame = CreateFrame(sequence: 2, [9, 8, 7]);
  ...
  private static (Stream Client, Stream Server) CreateConnectedStreams()
  {
    ...
    System.Net.Sockets.Socket server = listener.Accept();
    listener.Dispose();
    return (

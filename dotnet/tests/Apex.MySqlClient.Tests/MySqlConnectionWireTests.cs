...
public sealed class MySqlConnectionWireTests
{
  ...
  public async Task ConnectsAndExecutesSimpleTextQuery()
  {
    ...
    {
      await using FakeConnection connection = await harness.AcceptAsync();
    ...
    await using MySqlConnection client = await MySqlClient.ConnectAsync(harness.CreateOptions());
    SqlRowSet result = await client.QueryAsync("SELECT 1 AS id, 'hello' AS message");
  ...
  public async Task PreparesExecutesAndClosesStatement()
  {
    ...
    Task server = Task.Run(async () =>
    {
      ...
      byte[] executePayload = await connection.ExpectCommandPayloadAsync(MySqlCommand.StatementExecute);
      ...
      byte[] closePayload = await connection.ExpectCommandPayloadAsync(MySqlCommand.StatementClose);
    ...
    await using MySqlConnection client = await MySqlClient.ConnectAsync(harness.CreateOptions());
    await using ISqlPreparedStatement statement = await client.PrepareAsync("SELECT ? AS id");

    SqlRowSet rows = await statement.QueryAsync(SqlParameters.Create(42));
  ...
  public async Task ChainsMultipleResultSetsUsingMoreResultsExistsFlag()
  {
    ...
    await using MySqlConnection client = await MySqlClient.ConnectAsync(harness.CreateOptions());
    SqlRowSet first = await client.QueryAsync("CALL multi()");
  ...
  public async Task StreamsRowsThroughTheRowReader()
  {
    ...
    await using MySqlConnection client = await MySqlClient.ConnectAsync(harness.CreateOptions());
    List<int> values = [];
    await using (ISqlRowReader reader = await client.ExecuteReaderAsync("SELECT n FROM sequence"))
    {
  ...
  public async Task CancellationSendsKillQueryOnASecondConnectionAndReusesTheFirst()
  {
    ...
    Task server = Task.Run(async () =>
    {
      await using FakeConnection main = await harness.AcceptAsync();
      ...
      await using FakeConnection admin = await harness.AcceptAsync();
      await admin.CompleteHandshakeAsync("8.4.2", connectionId: 556);
      string killSql = await admin.ExpectTextCommandStartingWithAsync(MySqlCommand.Query, "KILL QUERY 555");
    ...
    ValueTask<SqlRowSet> pending = client.QueryAsync("SELECT SLEEP(10)", cancellation.Token);
    ...
    SqlRowSet rows = await client.QueryAsync("SELECT 1");
  ...
  public async Task PipelinedQueriesReceiveResponsesInSubmissionOrder()
  {
    ...
    await using MySqlConnection client = await MySqlClient.ConnectAsync(
    ...
    Task<SqlRowSet>[] queries = Enumerable.Range(0, count)
  ...
  private sealed class FakeConnection : IAsyncDisposable
  {
    ...
    {
      byte[] handshake = BuildHandshakePacket(serverVersion, connectionId, authPlugin);
    ...
    {
      byte[] payload = await ReadPacketAsync();
    ...
    {
      byte[] payload = await ExpectCommandPayloadAsync(command);
      string sql = Encoding.UTF8.GetString(payload.AsSpan(1));
    ...
    {
      int bitmapLength = (types.Length + 9) / 8;
    ...
    private Task WriteOkPacketAsync(bool moreResults)
    {
      ...
      try
      {
        ...
        writer.WriteLengthEncodedInteger(0);
        writer.WriteLengthEncodedInteger(0);
        ushort status = (ushort)MySqlServerStatus.AutoCommit;
        if (moreResults)
        {
    ...
    private static byte[] BuildHandshakePacket(string serverVersion, uint connectionId, string authPlugin)
    {
      ...
      try
      {
        ...
        writer.WriteBytes(Nonce.AsSpan(0, 8));
        writer.WriteByte(0);
        ...
        writer.WriteByte(21);
        writer.WriteZero(10);
        writer.WriteBytes(Nonce.AsSpan(8, 12));
        writer.WriteByte(0);
    ...
    {
      byte[] frame = new byte[payload.Length + MySqlProtocol.PacketHeaderLength];

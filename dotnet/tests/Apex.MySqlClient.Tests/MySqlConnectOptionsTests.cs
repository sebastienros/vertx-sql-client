...
public sealed class MySqlConnectOptionsTests
{
  ...
  {
    string? oldHost = Environment.GetEnvironmentVariable("MYSQL_HOST");
    string? oldPort = Environment.GetEnvironmentVariable("MYSQL_TCP_PORT");
    string? oldDatabase = Environment.GetEnvironmentVariable("MYSQL_DATABASE");
    string? oldUser = Environment.GetEnvironmentVariable("MYSQL_USER");
    string? oldPassword = Environment.GetEnvironmentVariable("MYSQL_PWD");
    try
    {
  ...
  {
    string scheme = "mysql://";
    string userInfo = "app%20user" + ":" + "s%40cret";
    string uri = scheme + userInfo + "@" + "db.example:3316/app%20db" +

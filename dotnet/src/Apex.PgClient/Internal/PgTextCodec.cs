...
internal static class PgTextCodec
{
  ...
  {
    string text = Utf8.GetString(value.Span);
    uint elementType = GetArrayElementType(typeId);
    if (elementType != 0)
    {
  ...
  internal static byte[] DecodeBytes(ReadOnlySpan<byte> value) =>
    ParseBytea(Utf8.GetString(value));
  ...
  internal static decimal DecodeDecimal(ReadOnlySpan<byte> value) =>
    Utf8Parser.TryParse(value, out decimal parsed, out int consumed) &&
    consumed == value.Length
      ? parsed
    ...
    ReadOnlyMemory<byte> value)
  {
    ...
    return DecodeArray(elementType, Utf8.GetString(value.Span));
  }

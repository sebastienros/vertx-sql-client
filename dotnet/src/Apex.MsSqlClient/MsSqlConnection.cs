...
public sealed class MsSqlConnection : ISqlConnection
{
    ...
    CancellationToken cancellationToken)
  {
    ...
    MsSqlConnectOptions current = options;
    ...
    [EnumeratorCancellation] CancellationToken cancellationToken)
  {
    ...
    await using (reader.ConfigureAwait(false))
    {
      ...
      while (true)
      {
        ...
        {
          SqlRowPageBatch fullBatch = page.BuildBatch(pageColumns!);
          page = null;
          for (int i = 0; i < fullBatch.Count; i++)
          {
        ...
        if (!await reader.ReadAsync(cancellationToken).ConfigureAwait(false))
        {
          if (page is not null)
          {
            SqlRowPageBatch finalBatch = page.BuildBatch(pageColumns!);
        ...
        int currentGeneration = reader.ResultSetGeneration;
        if (page is not null && currentGeneration != pageGeneration)
        {
          SqlRowPageBatch completedBatch = page.BuildBatch(pageColumns!);
          page = null;
          for (int i = 0; i < completedBatch.Count; i++)
          {
  ...
  internal async ValueTask<bool> WritePreparedRequestAsync(
    ...
    CancellationToken cancellationToken)
  {
    ReadOnlyMemory<byte> payload = statement.BuildRequest(
      parameters,
      Interlocked.Read(ref _transactionDescriptor),
  ...
  {
    ObjectDisposedException.ThrowIf(_disposed, this);
    string operation = GetOperation(statement.Sql);
  ...
  internal async ValueTask ClosePreparedAsync(MsSqlPreparedStatement statement)
  {
    ...
    await _scheduler.ExecuteAsync(
      async token =>
      {
        ...
        if (handle > 0)
        {
          ReadOnlyMemory<byte> payload = TdsRequestWriter.BuildUnprepare(
            handle,
            Interlocked.Read(ref _transactionDescriptor));
          await _writer.WriteMessageAsync(
            TdsMessageType.Rpc,
    ...
    CancellationToken cancellationToken)
  {
    ...
    try
    {
      ...
      TdsPreLoginResponse preLogin = await ExchangePreLoginAsync(
        stream,
  ...
  private static async ValueTask<Stream> UpgradeToTlsAsync(
    ...
    CancellationToken cancellationToken)
  {
    TdsTlsHandshakeStream? handshakeStream = encapsulateHandshake
      ? new TdsTlsHandshakeStream(stream, options.PacketSize)
      : null;
    Stream transport = handshakeStream ?? stream;
    ...
    X509CertificateCollection? certificates = options.ClientCertificates.Count == 0
      ? null
    ...
    CancellationToken cancellationToken)
  {
    ...
    while (!final)
    {
      ...
      {
        byte token = tokens.ReadTokenType();
        switch (token)
        {
          case TdsTokenType.LoginAck:
            TdsLoginAckInfo login = tokens.ReadLoginAck();
            ...
            break;
          case TdsTokenType.EnvironmentChange:
            TdsEnvironmentChangeInfo change = tokens.ReadEnvironmentChange();
          ...
          case TdsTokenType.DoneInProc:
            TdsDoneToken done = tokens.ReadDone();
  ...
  {
    using AttentionState attention = BeginAttention(cancellationToken);
    ...
    try
    {
      while (true)
      {
        TdsMessage message = await _reader.ReadMessageAsync(
        ...
        TdsQueryResponse response = parser.Parse(
          message.Payload,
          InvokeInfo,

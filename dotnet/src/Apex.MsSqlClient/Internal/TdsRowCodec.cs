...
internal static class TdsRowCodec
{
    ...
    TdsRowBuffer row)
  {
    ...
    ReadOnlySpan<byte> nullBitmap = nullCompressed
      ? reader.ReadSpan((columns.Count + 7) / 8)
      : default;
    for (int i = 0; i < columns.Count; i++)
    {
      bool isNull = nullCompressed && (nullBitmap[i >> 3] & (1 << (i & 7))) != 0;
    ...
    bool isNull)
  {
    ...
    switch (typeInfo.Type)
    {
      ...
      case TdsDataType.Image:
        ...
        reader.Skip(8);
        int dataLength = reader.ReadInt32LittleEndian();
  ...
  private static void WritePlp(
    ...
    ref TdsPayloadReader reader)
  {
    ulong totalLength = reader.ReadUInt64LittleEndian();
    ...
    while (true)
    {
      uint chunkLength = reader.ReadUInt32LittleEndian();
      if (chunkLength == 0)
      {
        break;
      }

...
internal sealed class MsSqlRowReader : ISqlRowReader, IValueTaskSource<bool>
{
  ...
  private async ValueTask<bool> PumpAsync()
  {
    ...
    bool final = false;
    MsSqlConnection.AttentionState attention =
    ...
    try
    {
      while (true)
      {
        byte messageType = await _connection.Reader.BeginMessageAsync(
        ...
        {
          byte token = await tokens.ReadTokenTypeAsync().ConfigureAwait(false);
          switch (token)
          {
            ...
            case TdsTokenType.DoneInProc:
              TdsDoneToken done = await tokens.ReadDoneAsync().ConfigureAwait(false);

...
internal static class TdsLogin7
{
  ...
  {
    string hostName = options.WorkstationId ?? Environment.MachineName;
    ...
    int dataLength = checked(
      ...
      11);
    byte[] login = new byte[checked(FixedLength + dataLength)];
    ...
    WriteExtensionPointer(login, 56, ref data, out int featureExtensionOffset);
    ...
    bool obfuscate)
  {
    ...
    byte[] bytes = obfuscate ? ObfuscatePassword(value) : Encoding.Unicode.GetBytes(value);

...
internal sealed class TdsPacketReader
{
  ...
  {
    byte type = await BeginMessageAsync(cancellationToken).ConfigureAwait(false);
    ...
    while (!EndOfMessage)
    {
      ...
      int payloadLength = _packetPayloadRemaining;
      Memory<byte> destination = payload.GetMemory(payloadLength)[..payloadLength];
    ...
    CancellationToken cancellationToken)
  {
    while (!destination.IsEmpty)
    {
      ...
      int count = Math.Min(destination.Length, _packetPayloadRemaining);
    ...
    CancellationToken cancellationToken)
  {
    ...
    while (length > 0)
    {
      int count = Math.Min(length, _skipBuffer.Length);
...
internal sealed class TdsPacketWriter : IDisposable
{
  ...
  {
    int maximumPayload = packetSize - 8;
    byte[] packet = reusablePacket is { Length: >= 512 } &&
    ...
    do
    {
      int count = Math.Min(maximumPayload, payload.Length - position);
      bool final = position + count == payload.Length;
      packet[0] = type;

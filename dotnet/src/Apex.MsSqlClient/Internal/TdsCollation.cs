...
{
  private static readonly IReadOnlyDictionary<int, Encoding> Encodings;

  static TdsCollationCodec()
  {
  ...
  {
    uint info = reader.ReadUInt32LittleEndian();
    byte sortId = reader.ReadByte();
    int codePage = unicode ? 1200 : ResolveCodePage(info, sortId);
  ...
  internal static Encoding GetEncoding(int codePage) =>
    Encodings.TryGetValue(codePage, out Encoding? encoding)
      ? encoding

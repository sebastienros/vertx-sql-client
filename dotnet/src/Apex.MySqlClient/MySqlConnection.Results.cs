...
public sealed partial class MySqlConnection
{
    ...
    CancellationToken cancellationToken)
  {
    ...
    {
      ResultHeader header = ReadResultHeader(packet.Span);
      if (header.IsLocalInfile)
      {
        string fileName = Utf8.GetString(packet.Span[1..]);
    ...
    MySqlRowDecoder decoder = await ReadColumnDefinitionsAsync(
      columnCount,
      binary,
  ...
  private async ValueTask<MySqlRowDecoder> ReadColumnDefinitionsAsync(
    ...
    CancellationToken cancellationToken)
  {
    MySqlColumnMetadata[] metadata = await ReadColumnMetadataAsync(columnCount, cancellationToken)
      .ConfigureAwait(false);
  ...
  private async ValueTask<MySqlColumnMetadata[]> ReadColumnMetadataAsync(
    ...
    CancellationToken cancellationToken)
  {
    MySqlColumnMetadata[] metadata = columnCount == 0
      ? []
    ...
    CancellationToken cancellationToken)
  {
    ...
    MySqlColumnMetadata[] columns = columnCount > 0
  ...
  private ResultHeader ReadResultHeader(ReadOnlySpan<byte> payload)
  {
    ...
    ulong count = reader.ReadRequiredLengthEncodedInteger();
  ...
  private bool TryCompleteResultSet(ReadOnlySpan<byte> payload)
  {
    ...
    MySqlCompletion completion = DeprecateEof
  ...
  private void EnsureEndOfMetadata(ReadOnlySpan<byte> payload)
  {
    ...
    MySqlCompletion completion = MySqlPackets.ReadEof(payload, _capabilities);
    _status = completion.Status;
  }
    ...
    ReadOnlySpan<byte> payload)
  {
    ...
    uint statementId = reader.ReadUInt32();

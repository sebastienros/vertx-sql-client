...
public sealed partial class MySqlConnection
{
    ...
    CancellationToken cancellationToken)
  {
    ...
    try
    {
      ...
      MySqlServerVersion version = ParseServerVersion(handshake.ServerVersion);
      bool unixSocket = IsUnixSocket(options);
      if (unixSocket &&
      ...
      bool serverSupportsTls =
        !unixSocket &&
        (handshake.Capabilities & MySqlCapabilities.Ssl) != 0;
      bool upgrade = options.SslMode switch
      {
      ...
      MySqlCapabilities capabilities = ComputeCapabilities(options, handshake, upgrade);
      byte sequence = handshake.Sequence;
      if (upgrade)
      {
        sequence++;
  ...
  {
    bool isMariaDb = serverVersion.Contains("MariaDB", StringComparison.OrdinalIgnoreCase);
    string normalized = isMariaDb
      ? serverVersion.Replace("5.5.5-", string.Empty, StringComparison.Ordinal)
      : serverVersion;
    int end = normalized.AsSpan().IndexOfAny('-', ' ');
    ReadOnlySpan<char> token = end < 0 ? normalized.AsSpan() : normalized.AsSpan(0, end);
    int major = 0;
    int minor = 0;
    int micro = 0;
    Span<Range> parts = stackalloc Range[4];
    int count = token.Split(parts, '.');
    if (count > 0)
    {
    ...
    bool upgradeToTls)
  {
    ...
    MySqlCapabilities negotiated = client & handshake.Capabilities;
  ...
  private static async ValueTask WriteSslRequestAsync(
    ...
    CancellationToken cancellationToken)
  {
    byte[] request = new byte[MySqlProtocol.PacketHeaderLength + 32];
    MySqlPacketReader.WriteHeader(request, 32, sequence);
    Span<byte> payload = request.AsSpan(MySqlProtocol.PacketHeaderLength);
  ...
  private async ValueTask AuthenticateAsync(
    ...
    CancellationToken cancellationToken)
  {
    byte[] password = MySqlAuthentication.GetPasswordBytes(_options.Password);
    try
    {
      ...
      while (true)
      {
        using MySqlPacket packet = await _reader.ReadAsync(cancellationToken).ConfigureAwait(false);
    ...
    AuthenticationState state)
  {
    ...
    byte header = payload.Length == 0 ? (byte)0 : payload[0];
    byte next = (byte)(sequence + 1);
    if (header == MySqlProtocol.OkHeader)
    {
      MySqlCompletion completion = MySqlPackets.ReadOk(payload, _capabilities);
    ...
    if (header == MySqlProtocol.EofHeader)
    {
      ...
      reader.Skip(1);
      string switched = reader.Remaining > 0
        ? reader.ReadNullTerminatedString()
        : MySqlProtocol.NativePasswordPlugin;
      ReadOnlySpan<byte> switchedNonce = reader.ReadRemainingSpan();
    ...
    AuthenticationState state)
  {
    ...
    {
      string publicKey = Utf8.GetString(payload[1..]).TrimEnd('\0');
      return state with
      {
    ...
    byte flag = payload.Length > 1 ? payload[1] : (byte)0;
  ...
  {
    int length = 0;
    foreach ((string key, string value) in _options.ConnectionAttributes)
    {
  ...
  private async ValueTask ApplySessionVariablesAsync(CancellationToken cancellationToken)
  {
    ...
    bool first = true;
    foreach ((string name, string value) in _options.SessionVariables)
    {
      if (name.Length == 0 ||
  ...
  private void HandleCompletionPacket(ReadOnlySpan<byte> payload)
  {
    ...
    MySqlCompletion completion = payload.Length > 0 && payload[0] == MySqlProtocol.EofHeader

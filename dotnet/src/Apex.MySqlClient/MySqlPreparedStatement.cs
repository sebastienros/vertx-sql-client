...
internal sealed class MySqlPreparedStatement : ISqlPreparedStatement
{
  ...
  {
    ObjectDisposedException.ThrowIf(_disposed, this);
    MySqlExecutionResult result = await _connection
    ...
    CancellationToken cancellationToken = default)
  {
    ...
    catch
    {
      ...
      {
        Task<SqlCommandResult> execution = pending[i];
        ...
        Exception failure = execution.Exception?.InnerExceptions.FirstOrDefault() ??

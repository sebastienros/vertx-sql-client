...
public sealed partial class MySqlConnection : ISqlConnection
{
  ...
  private MySqlCapabilities _capabilities;
    ...
    CancellationToken cancellationToken = default)
  {
    ...
    MySqlExecutionResult result = await ExecuteQueryCoreAsync(
      sql,
      default,
    ...
    [EnumeratorCancellation] CancellationToken cancellationToken = default)
  {
    ...
    {
      await foreach (SqlRow row in StreamTextRowsAsync(
                       sql,
                       fetchSize,
                       cancellationToken).ConfigureAwait(false))
      {
    ...
    await foreach (SqlRow row in StreamPreparedRowsAsync(
                     statement,
                     parameters,
    ...
    CancellationToken cancellationToken = default)
  {
    ObjectDisposedException.ThrowIf(_disposed, this);
    ArgumentException.ThrowIfNullOrWhiteSpace(sql);
    MySqlStatement statement = await PrepareCoreAsync(sql, cancellationToken).ConfigureAwait(false);
  ...
  public async ValueTask PingAsync(CancellationToken cancellationToken = default)
  {
    ...
    await _scheduler.ExecuteAsync(
      ...
      async _ =>
      {
        using MySqlPacket packet = await _reader.ReadAsync(CancellationToken.None)
          .ConfigureAwait(false);

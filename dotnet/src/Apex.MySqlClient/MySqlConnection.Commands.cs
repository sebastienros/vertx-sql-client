...
public sealed partial class MySqlConnection
{
    ...
    CancellationToken cancellationToken)
  {
    ...
    using Activity? activity = SqlClientDiagnostics.StartQuery(
      "mysql",
      _options.Database,
      _options.Host,
      _options.Port,
    ...
    CancellationToken cancellationToken)
  {
    ...
    MySqlStatement statement = await GetOrPrepareAsync(sql).ConfigureAwait(false);
    try
    {
      try
      {
  ...
  private ValueTask<MySqlExecutionResult> ExecuteStatementDirectAsync(
    ...
    ReceiveWithCancellationAsync(
      async () =>
      {
        ...
        await _writer.FlushAsync(CancellationToken.None).ConfigureAwait(false);
        SqlRowSet rows = await ReadResultsAsync(binary: true, CancellationToken.None)
          .ConfigureAwait(false);
  ...
  internal async ValueTask<SqlRowSet> ExecutePreparedAsync(
    ...
    CancellationToken cancellationToken)
  {
    MySqlExecutionResult result = await ExecutePreparedDetailedAsync(
      statement,
      parameters,
      cancellationToken).ConfigureAwait(false);
    return result.Rows;
  ...
  private ValueTask<MySqlExecutionResult> ReceiveExecutionWithCancellationAsync(
    ...
    ReceiveWithCancellationAsync(
      async () =>
      {
        SqlRowSet rows = await ReadResultsAsync(binary, CancellationToken.None)
          .ConfigureAwait(false);
        return new MySqlExecutionResult(rows, _lastCommandInfo);
      },
      cancellationToken);
  ...
  {
    await using MySqlRowReader reader =
      CreatePreparedReader(statement, parameters, ownsStatement, cancellationToken);
    await foreach (SqlRow row in StreamRowsAsync(reader, fetchSize, cancellationToken)
                     .ConfigureAwait(false))
    {
      yield return row;
    }
  ...
  private async ValueTask<ISqlRowReader> ExecutePreparedReaderCoreAsync(
    ...
    CancellationToken cancellationToken)
  {
    MySqlStatement statement = await GetOrPrepareViaSchedulerAsync(sql, cancellationToken)
      .ConfigureAwait(false);
  ...
  private async IAsyncEnumerable<SqlRow> StreamTextRowsAsync(
    ...
    [EnumeratorCancellation] CancellationToken cancellationToken)
  {
    await using MySqlRowReader reader = CreateTextReader(sql, cancellationToken);
    await foreach (SqlRow row in StreamRowsAsync(reader, fetchSize, cancellationToken)
                     .ConfigureAwait(false))
    {
      yield return row;
  ...
  private static async IAsyncEnumerable<SqlRow> StreamRowsAsync(
    ...
    [EnumeratorCancellation] CancellationToken cancellationToken)
  {
    int pageCapacity = Math.Min(fetchSize, 256);
    bool hasCurrent = await reader.ReadAsync(cancellationToken).ConfigureAwait(false);
    while (hasCurrent)
    {
      MySqlRowDecoder decoder = reader.Decoder;
      IReadOnlyList<SqlColumn> columns = reader.Columns;
      ...
      SqlRowPageBatch batch = page.BuildBatch(columns);
  ...
  private async ValueTask<MySqlStatement> GetOrPrepareAsync(string sql)
  {
    ...
    MySqlStatement prepared = await PrepareRawAsync(sql).ConfigureAwait(false);
  ...
  private void ClearStatementCache()
  {
    ...
    lock (_statementCacheGate)
    {
      foreach (MySqlStatement statement in _statementCache.DrainValues())
      {
  ...
  private void RemoveCachedStatement(string sql, MySqlStatement statement)
  {
    ...
    lock (_statementCacheGate)
    {
      if (_statementCache.TryGet(sql, out MySqlStatement? cached) &&
    ...
    CancellationToken cancellationToken)
  {
    ...
    CancellationTokenRegistration registration = cancellationToken.Register(state.Cancel);
    try
    {
      T result;
      try
  ...
  private async Task KillRunningQueryAsync()
  {
    ...
    try
    {
      MySqlConnectOptions adminOptions = _options with
      {
      ...
      await using MySqlConnection admin =

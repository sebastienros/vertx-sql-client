...
internal static class MySqlParameterEncoder
{
    ...
    int expectedCount)
  {
    ...
    Span<byte> bitmap = bitmapLength <= stack.Length
      ? stack[..bitmapLength]
    ...
    for (int i = 0; i < count; i++)
    {
      (MySqlType type, bool unsigned) = GetWireType(parameters[i]);
  ...
  private static void WriteTime(MySqlPayloadWriter writer, TimeSpan value)
  {
    ...
    long microseconds = absolute.Ticks % TimeSpan.TicksPerSecond / 10;
